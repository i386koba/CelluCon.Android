        ...
        switch (requestCode) {
            case REQUEST_ACCOUNT_CHOOSER:
                if ( resultCode == RESULT_OK && data != null ) {
                    ...
                    addTextView(peerView, "Google Login:miss!");
                    activityResult = "";
                }
                peerGetID();
                break;
            case REQUEST_AUTHORIZATION_FROM_DRIVE:
                if ( resultCode == RESULT_OK ) {
                    ...
                    //ログイン後再度GoogleドライブにPeerIDを書き込みに行く
                    new setDrivePeerIdTask().execute(mDrive, _id);
                } else {
                    // エラー処理
...
//  http://niudev.blogspot.jp/2012/02/blog-post.html
    ...
    private class setDrivePeerIdTask extends AsyncTask<Object, String, Boolean> {
        ...
        protected Boolean doInBackground(Object... params) {
            ...
            //この例外は、ユーザーが認証していない場合に発生、つまり初回は必ず発生する例外です。つまり、例外の情報は重要ではなく、この例外から取得可能なIntentが重要になります。ここではフィールドに保存して、後で利用するようにしています。
                ...
                //mIntent = e.getIntent();
                startActivityForResult(e.getIntent(), REQUEST_AUTHORIZATION_FROM_DRIVE);
                Log.e("AuthIOException", e.getMessage(), e);
                publishProgress("UserRecoverableAuthIOException");
            } catch (IOException e) {
                Log.e("doInBackground", e.getMessage(), e);
                publishProgress("doInBackground IOException:" +  e.getMessage());
            }
            return true;
        }
...
//        }
        ...
        protected void onProgressUpdate(String... values) {
            ...
            //追加した行を一番上に表示させる。
            String msg = values[0] + "\n";
            String addMsg = msg + peerView.getText();
            peerView.setText(addMsg);
            //textView.append(msg);
        }
    }
//AsyncTask非同期処理後のコールバック機能
